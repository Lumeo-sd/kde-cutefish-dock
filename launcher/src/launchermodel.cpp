...
// packaged desktop file, as the XDG basedir spec requires. Same source order
// as the dock's SystemAppMonitor.
    ...
    , m_firstLoad(false)
{
    ...
    // Watch every XDG application directory (user, flatpak exports, system),
    // not just /usr/share/applications, so per-user apps appear and live-update.
...
void LauncherModel::refresh(LauncherModel *manager)
{
    ...
    for (const QString &dir : applicationDirectories()) {
        ...
        while (it.hasNext()) {
            ...
            // applicationDirectories() is ordered user-first, so keep the
            // highest-priority copy (the app must not show up twice).
            if (seenIds.contains(QFileInfo(fileName).fileName()))
                continue;

            seenIds.insert(QFileInfo(fileName).fileName());
