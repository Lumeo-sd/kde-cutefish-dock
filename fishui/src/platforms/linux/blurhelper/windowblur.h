...
public:
     ...
     * (root.x/root.y/root.width/root.height) here; the blurred region is the
     * window's own surface (rounded by windowRadius), so the value no longer
     * has any effect. Kept as a no-op so the dock does not have to change.
     */
