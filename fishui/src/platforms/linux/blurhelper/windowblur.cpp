...
void WindowBlur::setGeometry(const QRect &geometry)
{
    if (m_geometry == geometry)
        return;

    // Legacy CutefishOS dock API: the dock QML binds a rectangle that tracked
    // the panel's position and size. The blur is applied to the window's own
    // surface and moved with it, so the value is not used anymore. Storing it
    // keeps the property self-consistent.
    m_geometry = geometry;
