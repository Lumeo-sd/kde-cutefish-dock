...
bool Appearance::darkMode() const
{
    ...
    // There is no CutefishOS appearance daemon on Plasma, so fall back to the
    // local override file. ~/.config/cutefishos/appearance.conf with
    //   [General]
    //   darkMode=true
    // lets the panel follow a dark style without the daemon.
    QSettings settings(QSettings::UserScope, QStringLiteral("cutefishos"), QStringLiteral("appearance"));
    return settings.value(QStringLiteral("darkMode"), false).toBool();
}
