...
import Cutefish.Appearance 1.0 as System
...
//        }
    ...
    RowLayout {
        ...
        StandardItem {
            ...
            RowLayout {
                ...
                Image {
                    ...
                    height: width
                    sourceSize: Qt.size(width, height)
                    // The framework's ActiveConnection only tracks the NM
                    // primary connection; when a VPN/dummy takes priority the
                    // icon name stays empty, so fall back to a generic one.
                    source: {
                        var icon = activeConnection.wirelessIcon
                        if (!icon)
                            icon = "network-wireless-connected-00"
                        return "qrc:/images/" + (rootItem.darkMode ? "dark/" : "light/") + icon + ".svg"
                    }
                    asynchronous: true
                    Layout.alignment: Qt.AlignCenter
                ...
                Image {
                    ...
                    height: width
                    sourceSize: Qt.size(width, height)
                    source: "qrc:/images/" + (rootItem.darkMode ? "dark/" : "light/") + "bluetooth-symbolic.svg"
                    asynchronous: true
                    Layout.alignment: Qt.AlignCenter
                    visible: bluetoothControl.available
                    opacity: bluetoothControl.powered ? 1.0 : 0.35
                    antialiasing: true
                    smooth: false
                }
        ...
        // Keyboard layout indicator (macOS style input source)
        StandardItem {
            id: languageItem

            animationEnabled: true
            Layout.fillHeight: true
            Layout.preferredWidth: languageLabel.implicitWidth + FishUI.Units.largeSpacing
