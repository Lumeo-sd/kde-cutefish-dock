    ...
    , m_serviceWatcher(new QDBusServiceWatcher(this))
{
    ...
    // Active window
    // On Wayland the application menu of the active window is announced by the
    // compositor through org_kde_kwin_appmenu and exposed per-window via
    // PlasmaWindow::applicationMenuServiceName/ObjectPath, replacing the X11
    // _KDE_NET_WM_APPMENU_* window properties.
    WindowManager *wm = WindowManager::instance();
    connect(wm, &WindowManager::activeWindowChanged, this, &AppMenuModel::onActiveWindowChanged, Qt::QueuedConnection);
    connect(wm, &WindowManager::windowStateChanged, this, &AppMenuModel::onActiveWindowChanged, Qt::QueuedConnection);
...
void AppMenuModel::onActiveWindowChanged()
{
    ...
    // The compositor tracks the application menu registration for the active
    // window (org_kde_kwin_appmenu) and hands the menu service object over to
    // us; the X11 _KDE_NET_WM_APPMENU_* window properties are gone on Wayland.
    const QString serviceName = wm->activeAppMenuServiceName();
    const QString objectPath = wm->activeAppMenuObjectPath();

    qInfo() << "AppMenuModel: service=" << serviceName << "path=" << objectPath
            << "menuAvailable=" << m_menuAvailable << "visible=" << m_visible;
